...
| LLM planners | Proposal-only planners: OpenRouter cloud, OpenAI-compatible API or local LLM (Ollama, LM Studio, llama.cpp, vLLM), Claude Code CLI, Codex CLI, agy. Strict schema, grounding to current actions and own evidence, redaction, caps. | Fixture and stand-in process tests only; no authenticated call to these engines in this build. Proposals never authorize actions; Jev decides. |
