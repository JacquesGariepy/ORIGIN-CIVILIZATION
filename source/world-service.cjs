...
class WorldService{
 ...
 async propose(request,{signal}){if(this.session.plannerAttempts>Number(this.env.SERVER_PLANNER_CAP||100))throw Error('Server planner cap reached.');const data=await this.planImpl(request,{signal,env:this.env,fetchImpl:this.fetchImpl});return {ok:true,status:200,json:async()=>JSON.parse(this.clean(data))};}
