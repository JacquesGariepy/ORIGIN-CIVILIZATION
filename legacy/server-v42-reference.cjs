...
function safeSnapshot(x){
 if(!x||!['origin-experiment-v3','origin-experiment-v4'].includes(x.format)||!Array.isArray(x.log))throw Error('Invalid checkpoint.');C.validateWorld(x.world);
 if(JSON.stringify(x).length>32*1024*1024)throw Error('Checkpoint exceeds 32 MB; export the ledger and start a new session.');return x;
}
function createServer({env=process.env,fetchImpl=global.fetch,planImpl=Planner.plan,dataDir=path.join(ROOT,'data'),recoveryPath=path.join(ROOT,'recovery','turn-74.snapshot.json')}={}){
 const token=crypto.randomBytes(32).toString('hex');let jevAttempts=0,plannerAttempts=0,connectionChecks=0,inFlight=false,saveQueue=Promise.resolve();
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname.startsWith('/api/')){
   ...
   let timer;const requestSecrets=[...secrets];try{
    ...
    if(inFlight){send(res,409,{error:'Another model request is active. Use a single ORIGIN tab.'});return;}
    inFlight=true;
    try{
